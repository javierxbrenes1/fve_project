...
class ADPedidos {
    ...
    public function InicializarPedido($pvcNombre,$pvcEmail,$pvcTelPrin,$pvcTelSec,
                                      $pvcDireccion,$pvnZona,$pvnTotProd,$pvnMontTotal, $pvcDiaEntrega, $pvcMedioPago)
    {
        $vlcScript = '';
        $vloAD;
        $vloCodigoPedido='';
        ...
        try {
            //Crea el script a ejecutar
                $vlcScript = "CALL spInicializaDatosPedido('".$pvcNombre."','".$pvcEmail."','".$pvcTelPrin."','".
                $pvcTelSec."','".$pvcDireccion."',".$pvnZona.",".$pvnTotProd.",".$pvnMontTotal.",'". $pvcDiaEntrega ."','".$pvcMedioPago."')";
